...
| `fork(source, at, target, agent, prompt=None, *, controls=None, rules_version="", source_conversation_id=None, conversation_id=None, deps=None, **options)` | Starts `target` from `source`'s transcript at entry uuid `at` (`None`: the active tip); requires a transcript. Effects the source finished in that history are copied to the new run's ledger and replay without being gated again; the rest runs under the new run's policy. The source is never written. |
